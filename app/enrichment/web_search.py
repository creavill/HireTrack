...
def extract_job_info_from_html(html: str) -> Dict[str, Any]:
    ...
    for pattern in requirements_markers:
        ...
        if match:
            ...
            items = re.split(r"[\n•\-\*]+", req_text)
            result["requirements"] = [
                item.strip() for item in items if item.strip() and len(item.strip()) > 10
            ][
                :10
            ]  # Max 10 requirements
            break
...
def search_job_posting(company: str, title: str, timeout: int = 10) -> WebSearchResult:
    """
    ...
    try:
        ...
        return WebSearchResult(
            found=False, enrichment_status="error", error="requests library not installed"
        )
    ...
    try:
        ...
        return WebSearchResult(
            found=False, enrichment_status="error", error=f"Search request failed: {str(e)}"
        )
...
def search_with_ai_enrichment(company: str, title: str, ai_provider: Any) -> Dict[str, Any]:
    """
